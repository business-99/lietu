...
docker run -p 8090:80 --name lietunginx --privileged=true --network=blabla -v $(pwd)/nginx/www:/www -v $(pwd)/static:/static -v $(pwd)/nginx/nginx.d:/etc/nginx/conf.d -v $(pwd)/nginx/log:/var/log/nginx -d nginx